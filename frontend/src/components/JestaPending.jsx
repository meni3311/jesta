...
function buildStages(employer) {
  return [
    { id: 0, text: `שולחים בקשה ל${employer}...`,   sub: "מעבירים את הפרופיל שלך",      icon: "📡", done: false },
    { id: 1, text: `${employer} בודק את הבקשה שלך`, sub: "בדרך כלל זה לוקח כמה דקות",   icon: "👀", done: false },
    { id: 2, text: "הבקשה התקבלה! 🎉",              sub: "נתראה במשמרת ⚡",              icon: "✅", done: true  },
  ];
}
      ...
      <div style={{ padding: "0 16px 16px" }}>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.45 }}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
            ...
            <div>
              <div style={{ fontSize: 13, fontWeight: 800, color: "#fff" }}>{employer}</div>
              <div style={{ fontSize: 11.5, color: "rgba(196,181,253,0.75)", fontWeight: 500 }}>
                {job?.employerRating ? `${job.employerRating.toFixed(1)} ⭐` : "מעסיק"}
              </div>
            </div>
          ...
          <AnimatePresence mode="wait">
            {rejected ? (
              <motion.span key="rejected" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                onClick={onBack} style={{ display: "flex", alignItems: "center", gap: 6, cursor: "pointer" }}>
                חזרה לפיד הג׳סטות ←
              </motion.span>
            ) : isDone ? (
